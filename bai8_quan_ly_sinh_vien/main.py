...
class SinhVien:
    def __init__(self, ma_sv, ho_ten):
        self.ma_sv = ma_sv
        self.ho_ten = ho_ten
        # Dictionary: Key là đối tượng MonHoc, Value là điểm số (float)
        self.bang_diem = {} 

    def them_diem(self, mon_hoc, diem):
        # mon_hoc phải là một đối tượng thuộc lớp MonHoc
    ...
    def diem_trung_binh(self):
        ...
        # Duyệt qua dictionary (Key là mon_hoc, Value là diem)
    ...
    # Nạp chồng toán tử == để so sánh 2 sinh viên
    def __eq__(self, other):
        return self.diem_trung_binh() == other.diem_trung_binh()
    
    # Khuyến khích: Nạp chồng thêm > và < để so sánh cho tiện (đề không bắt buộc nhưng nên có)
...
class SinhVienChinhQuy(SinhVien):
    ...
    def __str__(self):
        # Kế thừa chuỗi in của cha và thêm lớp học
